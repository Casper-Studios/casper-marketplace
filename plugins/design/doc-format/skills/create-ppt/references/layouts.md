...
4. **One argued content emphasis per slide.** Set `hot: true` or the accent stop when the layout exposes it.
   Repeated structural numerals and standard chrome are navigation furniture, not competing arguments.
...
`footnote` (both themes). One per deck, first.
...
> **All 16 content layouts call `sourceNote`.** The six that do not are the non-data slides where a footnote
