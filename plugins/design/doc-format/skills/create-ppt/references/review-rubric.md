...
- **Accent corner tick** — a short accent bar in the top-left corner only (x 0, y 0, w ~1.35, h ~0.09), *not*
  a full-width band. Present on standard content slides; absent by design on cover, section dividers, closing,
  and the two serif statement slides (`manifesto`, `quote`). A tick that stretches across the page would be a
  regression.
...
5. **One argued content emphasis.** The reader should encounter one primary conclusion, number, phase, or
   decision. Repeated structural numerals, the corner tick, kickers, and standard chrome do not count as
   competing emphasis. Multiple highlighted data marks fail only when they compete for the slide's argument.
   A layout that exposes `hot` should normally use it once.
...
13. **Capacity match — filled, or deliberately airy.** Compare what the layout is sized to hold against what it
...
- Preserve client-approved programme and workstream names. Remove internal codenames, unsanctioned workflow
  labels, automation percentages, and coverage-completeness claims.
