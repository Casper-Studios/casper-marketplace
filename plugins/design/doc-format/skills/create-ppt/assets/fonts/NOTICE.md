...
wght 400 and 700 during package maintenance. The variable source and maintainer-only regeneration
tooling are intentionally excluded from the runtime distribution. The source declares family
`"DM Sans 9pt"`, a near-miss name that caused `display: "DM Sans"` to silently fall back to Calibri
on any machine without DM Sans separately installed. Instancing is a modification under OFL-1.1
and is permitted under the same terms.
