#!/usr/bin/env node
// render.js — the deck engine. Wires a trusted, agent-authored content module to a theme and the
// layout library, emits a .pptx. This is skill infrastructure; it is not edited per deck.
//
// Usage:
//   node render.js <content.js> [out.pptx]
//
// content.js is executable JavaScript. Never run a module supplied by an untrusted source.
// The per-deck source of truth is the agent-authored content.js. Everything visual lives in
...
function buildDeck(content, outPath) {
  ...
  try {
    ...
    throw new Error(
      ...
      `  cd "${dir}" && npm ci\n\n` +
      `  One-time, ~10 seconds. Installs pptxgenjs, the only direct dependency. Everything else the\n` +
