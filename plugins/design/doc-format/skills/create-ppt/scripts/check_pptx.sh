...
command -v unzip >/dev/null 2>&1 || {
  echo "FAIL: unzip is required to inspect the PowerPoint package."
  exit 1
}

# Use an isolated, portable scratch directory rather than assuming a surrounding vault.
TEMP_ROOT="${TMPDIR:-/tmp}"
work="$(mktemp -d "$TEMP_ROOT/create-ppt-pptx.XXXXXX")"
